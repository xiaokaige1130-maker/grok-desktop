...
(function (global) {
  const STRINGS = {
    zh: {
      ...
      "mode.goalHint": "写好目标后按发送；可用 /goal status 查看进度",
      "mode.planEntered": "已进入计划模式 — 先讨论方案，确认后再改代码",
    ...
    en: {
      ...
      "mode.goal": "Goal mode",
      "mode.goalDesc": "Set a goal to pursue across turns (/goal)",
      "mode.goalShort": "Goal",
      "mode.task": "Task mode",
      "mode.taskDesc": "Normal execute & edit code (default)",
      "mode.taskShort": "Task",
      "mode.plan": "Plan mode",
      "mode.planDesc": "Explore a design first, then change code",
      "mode.planShort": "Plan",
      "mode.btnTitle": "Work mode",
      "mode.needSession": "Open or create a chat first, then switch mode",
      "mode.goalHint": "Write the goal, then Send. Use /goal status for progress.",
      "mode.planEntered": "Plan mode on — discuss the approach before editing code",
